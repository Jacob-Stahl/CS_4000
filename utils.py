...
def get_basic_stats(validation_data, pred_cols):
    validation_stats = pd.DataFrame()
    feature_cols = [c for c in validation_data if c.startswith("feature_")]
    for pred_col in pred_cols:
        # Check the per-era correlations on the validation set (out of sample)
...
def get_basic_TB200_stats(validation_data, pred_cols):
    validation_stats = pd.DataFrame()
    feature_cols = [c for c in validation_data if c.startswith("feature_")]
    for pred_col in pred_cols:
        # Check the per-era correlations on the validation set (out of sample)
        validation_correlations = fast_score_by_date(
            validation_data,
            [pred_col],
            TARGET_COL,
            tb=200,
        ...
        tb200_mean = validation_correlations.mean()[pred_col]
        tb200_std = validation_correlations.std(ddof=0)[pred_col]
        tb200_sharpe = tb200_mean / tb200_std
